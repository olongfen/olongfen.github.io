<template><div><h2 id="增删改查" tabindex="-1"><a class="header-anchor" href="#增删改查" aria-hidden="true">#</a> 增删改查</h2>
<pre><code>创建数据库表：
...
<h2 id="数据导出导入" tabindex="-1"><a class="header-anchor" href="#数据导出导入" aria-hidden="true">#</a> 数据导出导入</h2>
<ul>
<li>
<p>导出数据</p>
<p>导出数据和表结构:</p>
<pre><code>  mysqldump -u [用户名] -p [数据库名] &gt;&gt; 数据库名.sql
</code></pre>
<p>导出单张表结构和数据:</p>
<pre><code>  mysqldump -u [用户名] -p [数据库名] [表名] &gt;&gt; 表名.sql
</code></pre>
<p>导出表结构:</p>
<pre><code>  mysqldump -u [用户名] -p -d [数据库名]  &gt;&gt; 表名.sql
</code></pre>
<p>导出单张表结构:</p>
<pre><code>  mysqldump -u [用户名] -p -d [数据库名] [表名]  &gt;&gt; 表名.sql    
</code></pre>
</li>
<li>
<p>导入数据</p>
...
<p>方法2:</p>
<pre><code> mysql -u [用户名] -p  [数据库名] &lt; 数据库名.sql
</code></pre>
</li>
</ol>
</li>
